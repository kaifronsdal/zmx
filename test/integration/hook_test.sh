...
chk "hook file matches asset byte-for-byte" \
    'cmp -s "$HOME/.config/zmyth/hook.bash" src/lib/assets/hook.bash' \
    '$(diff $HOME/.config/zmyth/hook.bash src/lib/assets/hook.bash 2>&1 | head -5)'
...
if command -v zsh >/dev/null; then
  ...
  chk "zsh: matches asset" 'cmp -s "$HOME/.config/zmyth/hook.zsh" src/lib/assets/hook.zsh' ''
...
if command -v fish >/dev/null; then
  ...
  chk "fish: matches asset" 'cmp -s "$HOME/.config/zmyth/hook.fish" src/lib/assets/hook.fish' ''
...
out=$(env TERM=dumb bash --norc -c '. src/lib/assets/hook.bash; echo "v=$__ZMYTH_HOOK_V"' 2>&1)
chk "M5: TERM=dumb → hook inert (var unset)" 'echo "$out" | grep -q "^v=$"' '$out'
out=$(env TERM=xterm-256color bash --norc -c '. src/lib/assets/hook.bash; echo "v=$__ZMYTH_HOOK_V"' 2>&1)
