...
[ -x "$ZMYTH" ] || die "binary not found at $ZMYTH; build with: zig build zmyth"
...
# Wait until the session is hooked+idle (outer layer ready).
wait_idle() {
  local s=$1 i
  for i in $(seq 1 50); do
    "$ZMYTH" ls -j 2>/dev/null | grep -q "\"name\":\"$s\".*\"hooked\":true.*\"cmd_running\":false" && return 0
    sleep 0.1
  done
  return 1
}
...
mtime_before=$(stat -c %Y "$HOME/.config/zmyth/hook.bash" 2>/dev/null || stat -f %m "$HOME/.config/zmyth/hook.bash")
sleep 1.1
...
mtime_after=$(stat -c %Y "$HOME/.config/zmyth/hook.bash" 2>/dev/null || stat -f %m "$HOME/.config/zmyth/hook.bash")
chk "hook file not rewritten" '[ "$mtime_before" = "$mtime_after" ]' '$mtime_before vs $mtime_after'

"$ZMYTH" kill "$S" -9 2>/dev/null; sleep 0.2
