...
zmyth installs a function on each that emits an **OSC sequence** — `ESC ] ...
BEL`. Terminals render OSCs invisibly (they're how shells set the window
...
The opener is **drain-wrapped** — `head -c N | { base64 -d > path || cat >
/dev/null; }` — so a bad path (ENOENT, EACCES) doesn't leave N bytes of base64
...
2. **daemon** (child): `setsid`, redirect stdio to log, `Pty.open()`,
   `hook.spawnSpec(.bash, …)` → write `bashrc` shim → `forkExec(bash --rcfile
   shim -i)`, `Session.init()`, bind+listen, enter `poll()` loop.
...
10. **daemon** `routeEvents()`: `drainEvents()` returns one `.run_done{cookie=
    client_id, ec=7, via=.pty_eof}`. Send IPC `.run_done` to that client.
    Send `.eof` to all clients. Clean up socket/lock/rc-dir, exit.
