...
pub const Session = struct {
    ...
    pub fn checkPromptWait(self: *Session, now_ns: i128) PromptWait {
        ...
        // is queued behind a running command which may legitimately take
        // hours — never fail it, just warn.
        if (self.layers.len == 0 and waited >= 30 * std.time.ns_per_s) {
            const req = self.removeReq(r);
